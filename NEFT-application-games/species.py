...
class Species:
    ...
    def similarity(self, brain):
        similarity_res = self.weight_difference(self.benchmark_brain, brain)
        return self.threshold > similarity_res
    ...
    def weight_difference(brain_1, brain_2):
        ...
        for i in range(0, len(brain_1.connections)):
            for j in range(0, len(brain_2.connections)):
                if i == j:
                    total_weight_difference += abs(brain_1.connections[i].weigth -
                                                    brain_2.connections[j].weigth)
